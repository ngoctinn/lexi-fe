...
export function InstantLookup() {
  ...
  const memoizedVirtualRef = React.useMemo(
    () => ({
      current: { getBoundingClientRect: () => virtualRef || new DOMRect() },
    }),
    [virtualRef],
  );
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverAnchor virtualRef={memoizedVirtualRef} />
          ...
          <CardHeader className="pb-3 px-4 pt-4 border-b bg-muted/30">
            <div className="flex items-start justify-between">
              <div className="space-y-1">
                <CardTitle className="text-lg font-bold text-primary break-all">
                  {word}
                </CardTitle>
                <div className="flex items-center gap-2">
                  <Badge
                    variant="secondary"
                    className="px-1.5 py-0 rounded-sm text-[10px] uppercase font-semibold"
                  >
                    Noun
                  </Badge>
          ...
          <CardContent className="space-y-4 p-4">
            <div className="text-sm">
              ...
              <p className="text-muted-foreground leading-relaxed">
                (Bản dịch cho {`"${word}"`} sẽ hiển thị ở đây sau khi kết nối
                module Dictionary).
              </p>
            </div>
